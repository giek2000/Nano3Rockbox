...
/* This target's NAND is driven entirely by our own nand-nano3g.c/
 * ftl-nano3g.c (selected unconditionally under #ifdef IPOD_NANO3G in
 * firmware/SOURCES), not by any of the generic Rockbox NAND storage
 * backends (ata-nand-telechips.c, ata-nand-s5l8700.c, ata-nand-rk27xx.c)
 * those select via CONFIG_NAND. Even so, firmware/SOURCES compares
 * CONFIG_NAND against NAND_TCC/NAND_SAMSUNG/NAND_RK27XX unconditionally
 * while selecting those other targets' sources, and that comparison runs
 * during every target's build (SOURCES is shared) -- leaving CONFIG_NAND
 * undefined here triggers -Wundef warnings on those lines. 0 matches
 * none of NAND_TCC/NAND_SAMSUNG/NAND_RK27XX, so it silences the warnings
 * without selecting any of those unused backends. */
...
/* The Nano 3G's factory battery (Apple part 616-0311/616-0333/616-0337) is
 * a 450 mAh, 3.7 V Li-polymer cell -- consistently reported by iFixit and
 * multiple independent aftermarket battery vendors selling exact
 * replacements for that part number. Third-party replacements are sold up
 * to ~520 mAh. */
#define BATTERY_CAPACITY_DEFAULT 450 /* default battery capacity */
