...
static void handle_scsi(struct command_block_wrapper* cbw)
{
    ...
    switch (cbw->command_block[0]) {
        ...
        case SCSI_SYNCHRONIZE_CACHE_10:
            /* Hosts issue this after writes and around partition/format
             * operations. It previously fell through to the default case,
             * which answers UMS_STATUS_FAIL / SENSE_ILLEGAL_REQUEST --
             * observed on real hardware (iPod Nano 3G) to make Windows
             * abort Clear-Disk with a generic storage-service failure
             * even though every READ/WRITE had succeeded. Honour it
             * instead: commit anything outstanding and report the result.
             * Targets without HAVE_STORAGE_FLUSH have nothing to commit,
             * so succeeding unconditionally is correct for them. */
