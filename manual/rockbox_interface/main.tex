...
\blind{%
  ...
  %
  \opt{ipod6g}{
  Earphones with an Apple-compatible inline remote are supported (not on
  the 2007 model). A click of the remote's center button toggles
  play/pause on any screen, even while the \ButtonHold{} switch is on;
  the center button has no long-press function. The remote's volume
  buttons adjust the \dap{}'s volume everywhere.
  }
  %
  \opt{ipod3g}{
...
\nopt{gigabeatf,iaudiom3,iaudiom5,iaudiox5,rgnano}
  ...
  \opt{ipodnano3g}
    {
    Rockbox has a dual-boot feature. To boot into the original firmware, hard
    reset the player by pressing and holding \ButtonMenu{} and
    \ButtonSelect{}, then release \ButtonSelect{} and keep holding
    \ButtonMenu{} until the Apple logo appears.

    You can return to Rockbox by pressing and holding \ButtonMenu{} and
    \ButtonSelect{} simultaneously until the player hard resets.
    }
